...
public:
  ...
  auto apply_1q = [&](auto fn_per_wire) {
    if (ops.size() != 1) throw std::runtime_error(g + " expects 1 operand");
    for (int q : ops[0]) fn_per_wire(q);
  };
  auto apply_2q_pairwise = [&](auto fn_pair) {
    if (ops.size() != 2) throw std::runtime_error(g + " expects 2 operands");
    if (ops[0].size() != ops[1].size()) throw std::runtime_error("mismatched arity for pairwise 2-qubit op");
...
// returns absolute wire indices for q[...] (handles single index, range a:b, set {i,j,...})
// returns absolute wire indices for q[...] (handles single index, range a:b, set {i,j,...}, and lists)
// returns absolute wire indices for q[...] (handles single index, range a:b[:s], set {i,j,...}, and lists)
std::vector<int> expandIndexedIdentifier(qasm3Parser::IndexedIdentifierContext* iq) {
  const std::string qname = iq->Identifier()->getText();

  if (iq->indexOperator().empty()) {
    throw std::runtime_error("array operand without index not supported yet: " + qname);
  }
  auto* op = iq->indexOperator(0);
  ...
  if (auto* s = op->setExpression()) {
    ...
    for (auto* e : s->expression()) out.push_back(rt_.qubitIndex(qname, (int)evalInt(e)));
    return out;
  }
  ...
  if (op->expression().size() > 1) {
    ...
    for (auto* e : op->expression()) out.push_back(rt_.qubitIndex(qname, (int)evalInt(e)));
    return out;
  }
...
std::vector<std::vector<int>> collectOperandVectors(qasm3Parser::GateOperandListContext* gl) {
  std::vector<std::vector<int>> out;
  for (auto* go : gl->gateOperand()) out.push_back(gateOperandToIndices(go));
  return out;
}
...
double evalReal(qasm3Parser::ExpressionContext* e) {
  ...
  // bare identifier: allow ints/bools
  {
    std::string name;
    if (exprIsBareIdentifier(e, name)) {
      if      (rt_.hasInt(name))  return (double)rt_.getInt(name);
      else if (rt_.hasBool(name)) return rt_.getBool(name) ? 1.0 : 0.0;
      // fallthrough -> error
    }
  }
  // (expr)
  if (auto* par = dynamic_cast<P::ParenthesisExpressionContext*>(e)) return evalReal(par->expression());
  // unary
  if (auto* un = dynamic_cast<P::UnaryExpressionContext*>(e)) {
