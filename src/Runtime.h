...
public:
  ...
  void allocQubits(const std::string &name, int n) {
    if (n <= 0)
      throw std::runtime_error("allocQubits: size must be > 0");
    if (qregs_.count(name))
      throw std::runtime_error("qubit register '" + name + "' already exists");
    int base = numQubits_;
    qregs_[name] = {base, n};
    numQubits_ += n;
  ...
  int qubitIndex(const std::string &qname, int idx) const {
    ...
    if (it == qregs_.end())
      throw std::runtime_error("unknown qubit register '" + qname + "'");
  ...
  void setBit(const std::string &name, int i, int v) {
    ...
    if (it == bits_.end())
      throw std::runtime_error("unknown bit var '" + name + "'");
    if (i < 0 || i >= (int)it->second.size())
  ...
  void cx(int c, int t) {
    if (c == t)
      throw std::runtime_error("control and target must differ");
    const size_t N = psi_.size();
  ...
  void apply1(int q, std::complex<double> u00, std::complex<double> u01,
